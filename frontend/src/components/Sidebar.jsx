...
const Sidebar = () => {
    ...
    // Helper to check if an admin tab is active based on search query parameters
    const isAdminTabActive = (tabName) => {
        const params = new URLSearchParams(location.search);
        const activeTab = params.get("tab") || "users";
        return location.pathname === "/admin" && activeTab === tabName;
    };
            ...
            <div className="sidebar-nav">
                ...
                {user.role === "student" && (
                    <>
                        ...
                        <NavLink to="/ai-assistant" className={({ isActive }) => `sidebar-link ${isActive ? "active" : ""}`}>
                ...
                {user.role === "admin" && (
                    <>
                        ...
                        <NavLink to="/admin?tab=users" className={() => `sidebar-link ${isAdminTabActive("users") ? "active" : ""}`}>
                            <svg className="sidebar-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                        ...
                        <NavLink to="/admin?tab=courses" className={() => `sidebar-link ${isAdminTabActive("courses") ? "active" : ""}`}>
                            <svg className="sidebar-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                                <path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20M4 19.5A2.5 2.5 0 0 0 6.5 22H20M4 19.5V3.5A2.5 2.5 0 0 1 6.5 1H20v20H6.5a2.5 2.5 0 0 1-2.5-2.5z"/>
                            </svg>
                            <span>Manage Courses</span>
                        </NavLink>
                        <NavLink to="/admin?tab=enrollments" className={() => `sidebar-link ${isAdminTabActive("enrollments") ? "active" : ""}`}>
                            <svg className="sidebar-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                        ...
                        <NavLink to="/admin?tab=faculties" className={() => `sidebar-link ${isAdminTabActive("faculties") ? "active" : ""}`}>
                            <svg className="sidebar-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                        ...
                        <NavLink to="/admin?tab=audit-logs" className={() => `sidebar-link ${isAdminTabActive("audit-logs") ? "active" : ""}`}>
                            <svg className="sidebar-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
